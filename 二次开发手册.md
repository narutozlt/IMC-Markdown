...
### 如何将定制开发的jar包加载到general微服务中


>适用于需要自定义IMC框架中schema模块的切片方法（例如：OnCreate,OnUpdate,OnDelete等等）</font>


#### 步骤说明

1. 将定制开发的schema部分单独作为一个module，包名必须为：***com.imc.schema.interfaces*** </br>

2. 把编译后的定制schema模块的jar包拷贝到imc部署目录的general文件夹下

3. 在启动jar包命令中引入定制开发应用的schema模块jar包（以imc-qms-schema.jar示例），修改general-start.bat文件为以下内容：
   ...
   REM java -Dfile.encoding=utf-8 %JAVA_OPTS% -jar imc-general.jar
   java -Dfile.encoding=utf-8 %JAVA_OPTS% -cp imc-general.jar -Dloader.path=imc-qms-schema.jar -Dloader.main=com.imc.general.IMCGeneralApplication org.springframework.boot.loader.PropertiesLauncher
   
   pause
   ```

4. 运行general-start.bat文件